...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x, return_all=False, local_group_memory_inputs=None, **kwargs):
        ...
        if self.num_relation_blocks > 0:
            ...
            rx = self.get_similarity_patches(x)
            if rx is not None:
                rx = torch.cat((x[:, 0].unsqueeze(1), rx), dim=1)
            else:
                rx = x
    ...
    def _get_similarity_index(self, x):
        ...
        self.top_n = 10
        cosine = torch.empty((x.shape[0], x.shape[1]-1)).to(x[0].device)
        for i in range(x.shape[0]):
            cosine[i, :] = x[i, 0] @ x[i, 1:].T
        sim, index = torch.topk(cosine, self.top_n, dim=-1)
        return sim, index

    @torch.no_grad()
    def get_similarity_patches(self, x, index=None):
        """
        compute top-n neighbors (key-value pair) in memory
        """
        if index is None:
            sim, index = self._get_similarity_index(x)
        
        get_k = torch.empty((x.shape[0], index.shape[-1], x.shape[2])).to(x[0].device)
        for i in range(x.shape[0]):
            get_k[i, :] = x[i, index[i, :]+1, :]
        
        # get_k = self.queue_k[index.view(-1)] # index [2, 8], get_k [16, 384]
        # get_v = self.queue_v[index.view(-1)]
        # B, tn = index.shape
        # get_k = get_k.view(B, tn, self.dim) # get_k [2, 8, 384]
